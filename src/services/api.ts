...
import {
  ...
  openInBrowser,
  saveImageWithProperPath,
} from "../utils/file-system.js";

export const API_BASE_URL = "https://api.context3d.ai";

interface GenerateRodinResponse {
  error?: string;
  message: string;
  uuid: string;
  jobs: {
 ...
 */
export async function downloadResults({ task_uuid }: any): Promise<any> {
  try {
    // Download results
    const response = await axios.post(
    ...
    const { savedPath } = await saveImageWithProperPath(
      fileResponse.data,
      outputFileName
    );
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error("Response data:", error.response?.data);
      throw new Error(
        `Failed to download Rodin results: ${
          error.response?.data?.message || error.message
        }`
      );
    } else {
